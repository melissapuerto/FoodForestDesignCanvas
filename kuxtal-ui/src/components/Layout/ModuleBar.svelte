<script lang="ts">
  ...
  const groups = $derived<Group[]>([
    {
      ...
      items: [
        {
          id: 'plantas',
          label: t('module_plants'),
          icon: 'Seed',
          sub: t('module_plants_sub')
        },
        {
          id: 'lienzo',
          label: t('module_lienzo'),
          icon: 'Map',
          sub: t('module_lienzo_sub')
        },
        {
          id: 'animales',
          label: t('module_animals'),
          icon: 'Mariposa',
          sub: t('module_animals_sub')
        },
        {
          id: 'recursos',
          label: t('module_resources'),
          icon: 'Box',
          sub: t('module_resources_sub')
        }
      ]
    },
    {
      ...
      items: [
        {
          ...
          label: t('module_log'),
          icon: 'Mic',
          sub: t('module_log_sub')
        },
        {
          id: 'protocolo',
          label: t('module_protocol'),
          icon: 'List',
          sub: t('module_protocol_sub')
        },
        {
          id: 'cosecha',
          label: t('module_harvest'),
          icon: 'Basket',
          sub: t('module_harvest_sub')
        }
      ]
    },
    {
      ...
      items: [
        {
          id: 'heredado',
          label: t('module_heredado'),
          icon: 'Book',
          sub: t('module_heredado_sub')
        },
        {
          id: 'saberes',
          label: t('module_rules'),
          icon: 'Sparkle',
          sub: t('module_saberes_sub')
        },
        {
          id: 'calendarios',
          label: t('module_calendars'),
          icon: 'Sun',
          sub: t('module_calendars_sub')
        }
      ]
    },
    {
      ...
      items: [
        {
          id: 'comunidad',
          label: t('module_community'),
          icon: 'People',
          sub: t('module_community_sub')
        }
      ]
    },
    {
      ...
      items: [
        {
          id: 'analisis',
          label: t('module_codex'),
          icon: 'Chart',
          sub: t('module_codex_sub')
        },
        {
          id: 'ajustes',
          label: t('topbar_tweaks'),
          icon: 'Settings',
          sub: t('module_settings_sub')
        }
      ]
    }
  ])
  ...
  async function openPopoverGroup(g: Group): Promise<void> {
    ...
    const arrow = Math.min(
      80,
      Math.max(20, ((btnCenter - finalPopLeft) / pr.width) * 100)
    )
  ...
  $effect(() => {
    ...
    untrack(() => {
      if (!a) return
      const grp =
        groups.find((g) => g.items.some((i) => i.id === a))?.id ?? null
  ...
  function setNavHeight(node: HTMLElement): void {
    const occupied = Math.max(
      0,
      Math.round(window.innerHeight - node.getBoundingClientRect().top)
    )
...
<div
  class="module-bar-shell"
  class:collapsed
  bind:this={containerEl}
  use:publishNavHeight
>
  {#if !srMode}
    <button
      type="button"
    ...
    >
      <span
        class="nav-handle-ico"
        aria-hidden="true">{$navCollapsed ? '▴' : '▾'}</span
      >
      {#if $navCollapsed}<span class="nav-handle-lbl">{t('nav_menu')}</span
        >{/if}
    </button>
  {/if}
  <nav
      ...
      >
        ...
        {#if !single && isOpen}
          <div
            ...
            style:--arrow-left={arrowPct[g.id] != null
              ? arrowPct[g.id] + '%'
              : '50%'}
          >
            <div
...
<style>
  .module-bar-shell {
    ...
    display: flex;
    flex-direction: row;
    align-items: flex-end; /* Align bottom so they overflow upwards if height is 0 */
  ...
  /* Collapsed: drop height to 0 so getBoundingClientRect() returns 0 for publishNavHeight, 
     freeing up screen space without absolute positioning the handle. */
  .module-bar-shell.collapsed {
    height: 0;
  }

  /* Pull-handle: now a detached pill on the left side of the bar. */
  .nav-handle {
    flex-shrink: 0;
    pointer-events: auto;
    display: inline-flex;
    ...
    box-shadow:
      0 0 0 2px var(--paper),
      0 0 0 3px oklch(0.55 0.12 50 / 0.3),
      0 4px 12px oklch(0.2 0.04 60 / 0.15);
    transition:
      background 0.18s var(--ease-codex),
      transform 0.18s var(--ease-codex);
  }
  .nav-handle:hover {
  ...
  @keyframes popUp {
    from {
      opacity: 0;
      transform: translateX(calc(-50% + var(--nudge-x, 0px))) translateY(4px);
    }
    to {
      opacity: 1;
  ...
  .popover-item.on .popover-sub {
    color: oklch(0.92 0.05 70);
  }

  :global(.a11y-large-text) .mod-btn {
    font-size: calc(12px * var(--text-scale));
    padding: 10px 16px;
    min-width: 84px;
  }
  :global(.a11y-large-text) .popover-label {
    font-size: calc(15px * var(--text-scale));
  }
  :global(.a11y-large-text) .popover-sub {
    font-size: calc(11px * var(--text-scale));
  }
  ...
  @media (max-width: 760px) {
    .module-bar-shell {
      ...
      padding: 0 calc(8px + var(--safe-right)) 0 calc(8px + var(--safe-left));
    }
    .module-bar-shell.collapsed .nav-handle {
      ...
      background: var(--paper); /* Ensure solid background */
    }
    .module-bar-shell.collapsed .nav-handle-lbl {
      display: none;
    }
    .module-bar-shell.collapsed .nav-handle-ico {
      font-size: 16px;
      margin-top: 0;
    }
    .module-bar {
